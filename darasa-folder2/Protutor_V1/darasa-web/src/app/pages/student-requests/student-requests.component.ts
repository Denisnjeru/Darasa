...
export class StudentRequestsComponent implements OnInit {
  ...
  user: User;
  requests: Request[] = [];
  selectedFilters: any = { status: 'pending' };
  filters: any = [
  ...
  filterRequests(selectedFilters = this.selectedFilters): void {
    if (this.user?.id) {
      ...
      this.requestsService
        .getUserRequests(this.user?.id, params)
        .subscribe(response => {
  ...
  searchRequests(): void {
    if (this.searchText.length >= 2 && this.user?.id) {
      this.loadingRequests = true;
      this.requestsService
        .getUserRequests(this.user?.id, { search: this.searchText })
        .subscribe(response => {
          this.collectionSize = response.count;
          this.requests = response.results;
