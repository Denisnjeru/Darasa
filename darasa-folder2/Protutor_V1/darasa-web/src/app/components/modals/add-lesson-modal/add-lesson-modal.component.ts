...
export class AddLessonModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.lessonForm = this.formBuilder.group({
      ...
      notes: [null],
      parent_lesson: [null],
      position: [this.course.lessons.length + 1 || 0],
    });
  }
  ...
  onSubmit(): void {
    ...
    this.coursesService
      .createLesson(lesson)
      .subscribe(newLesson => {
        if (newLesson?.parent_lesson !== null) {
          this.course.lessons.map(l => {
            if (l.id === newLesson?.parent_lesson) {
              l.children_lessons.push(newLesson);
            }
          });
        } else {
